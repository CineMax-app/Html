...
// Función para obtener la clasificación por edad de la película o serie
async function getClassification(id, type) {
    let classification = 'N/A'; // Valor por defecto
    ...
    try {
        ...
        if (type === 'movie') {
            ...
            url = `https://api.themoviedb.org/3/tv/${id}/release_dates?api_key=${api_key}`;
        }

        const response = await fetch(url);
        if (response.status === 200) {
            const data = await response.json();
            // Buscar la clasificación para el país "US" (Estados Unidos), por ejemplo
            const certification = data.results.find(item => item.iso_3166_1 === "US");

            if (certification && certification.release_dates.length > 0) {
                // Obtener la clasificación de la fecha de estreno
                classification = certification.release_dates[0].certification || 'N/A';
            }
        }
    } catch (error) {
...
async function displaySearchResults(results) {
    ...
    if (results.length > 0) {
        results.forEach(async (result) => {
            ...
            // Obtener el título o nombre
            const title = result.title || result.name;
            const classification = await getClassification(result.id, result.media_type); // Obtener la clasificación por edad

            // Crear el HTML de cada resultado
            resultItem.innerHTML = `
                <h3>${title}</h3>
                <p>Clasificación por edad: ${classification}</p>
            `;
    ...
    const cargarPeliculas = async () => {
        if (isSerie.checked) {
            try {
                ...
                <p>Created by: ${serieData.created_by.map(creator => creator.name).join(', ')}</p>
                </div>
                </div>
                </div>
                <!-- Ventana Modal Video -->
            ...
            try {
                ...
                if (respuesta.status === 200) {
                    ...
                    const modalOptions = [
                        userInputA ? `<a href="https://player-cinemax.ct.ws/app/supervideo.php?v=${userInputA}&secure_uri=true" class="modal-option" id="option1">
                            <i class="fas fa-play"></i> SUPERVIDEO SD
                        </a>` : '',
                        userInputB ? `<a href="https://player-cinemax.ct.ws/app/dropload.php?v=${userInputB}&secure_uri=true" class="modal-option" id="option2">
                            <i class="fas fa-play"></i> DROPLOAD SD
                        </a>` : '',
                        userInputC ? `<a href="https://player-cinemax.ct.ws/app/streamwish.php?v=${userInputC}&secure_uri=true" class="modal-option" id="option3">
                            <i class="fas fa-play"></i> HLSWISH FHD
                        </a>` : '',
                        userInputD ? `<a href="https://player-cinemax.ct.ws/app/filegram.php?v=${userInputD}&secure_uri=true" class="modal-option" id="option4">
                            <i class="fas fa-play"></i> FILEGRAM FHD
                        </a>` : ''
                    ].filter(Boolean).join('');
...
<li><i class="fas fa-user-shield" style="color: #28a745;"></i> Clasificación: ${clasificacion}</li>
</ul>
</div>
</div>
